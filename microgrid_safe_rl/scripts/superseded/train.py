...
# ------------------------- CLI ------------------------- #
def parse_args():
    p = argparse.ArgumentParser(description="Train RL agent on microgrid environments")
    
    # Environment selection
    p.add_argument("--env_id", type=str, required=True,
                   help="Environment ID (case33, case145, case300, case1888)")
    ...
    p.add_argument("--save_dir", type=str, required=True,
                   help="Directory to save models (e.g., artifacts/models/case33_run1)")
    p.add_argument("--tb_logdir", type=str, required=True,
                   help="TensorBoard log directory (e.g., artifacts/runs/case33_run1)")
    p.add_argument("--init_model", type=str, default=None,
                   help="Path to .zip model to continue training from")
    
    # Config files (required for flexibility)
    p.add_argument("--agent_cfg", type=str, required=True,
                   help="Agent config YAML (e.g., configs/agent/ppo.yaml)")
    p.add_argument("--env_cfg", type=str, required=True,
                   help="Environment config YAML (e.g., configs/env_case33.yaml)")
    p.add_argument("--scenario_cfg", type=str, required=True,
                   help="Scenario config YAML (e.g., configs/scenario/train/baseline.yaml)")
    
    # Algorithm override
    p.add_argument("--algo", choices=list(ALGOS.keys()), default=None,
                   help="Override algorithm from agent_cfg (ppo/sac)")
    
    # Vectorization
    p.add_argument("--n_envs", type=int, default=4,
                   help="Number of parallel environments (use CPU cores)")
    p.add_argument("--vec_backend", choices=["dummy", "subproc"], default="subproc",
                   help="Vectorization backend (subproc for multi-core)")
    ...
    p.add_argument("--log_interval", type=int, default=10,
                   help="Log to TensorBoard every N episodes")
    
    # Checkpointing and evaluation (in timesteps)
